...
class PostgresMissionRunRepositoryAdapter(MissionRunRepository):
    ...
    async def create(self, run: MissionRun) -> MissionRun:
        stmt = (
            ...
            .values(
                ...
                last_frame_at=run.last_frame_at,
    ...
    async def touch_last_frame(self, run_id: UUID, when: datetime) -> None:
        await self._session.execute(
            update(MissionRunRow).where(MissionRunRow.run_id == run_id).values(last_frame_at=when)
        )
...
def _to_run(row: MissionRunRow) -> MissionRun:
    return MissionRun(
