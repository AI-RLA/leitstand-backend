...
Dispatches missions to robots via queryables (proto-canonical JSON payloads);
pause/resume via PUT keys. Cancel/pause/resume are fire-and-confirm: the effect
is observed on the state channel, not confirmed by the reply value.
"""
...
class ZenohMissionDispatcherAdapter(MissionDispatcher):
    """Serialize a mission to proto-JSON, call the robot's queryable, parse the reply."""
    ...
    async def dispatch(self, mission: Mission, robot_id: str) -> None:
        ...
        payload = _to_json(dispatch_request_to_proto(mission, dispatch_id))
        ...
        if not replies:
            logger.warning(
                "mission_dispatch_timeout",
                mission_id=str(mission.mission_id),
                robot_id=robot_id,
            )
            raise MissionDispatchTimeout(mission.mission_id, robot_id)
        ...
        for reply in replies:
            ...
            if not accepted:
                raise MissionRejectedByRobot(mission.mission_id, robot_id, reason)
            logger.info(
                "mission_dispatch_accepted",
                mission_id=str(mission.mission_id),
                robot_id=robot_id,
            )
            return
    ...
    async def cancel(
        ...
        payload = _to_json(cancel_to_proto(mission_id, mode))
        logger.info(
            "mission_cancel_sending", mission_id=str(mission_id), robot_id=robot_id, mode=mode.value
        )
    ...
    async def pause(self, mission_id: UUID, robot_id: str) -> None:
        ...
        logger.info("mission_pause_sending", mission_id=str(mission_id), robot_id=robot_id)
    ...
    async def resume(self, mission_id: UUID, robot_id: str) -> None:
        ...
        logger.info("mission_resume_sending", mission_id=str(mission_id), robot_id=robot_id)
        await _put(self._session, key)
