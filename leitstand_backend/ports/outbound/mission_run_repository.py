...
class MissionRunRepository(ABC):
    ...
    async def get_for_update(self, run_id: UUID) -> MissionRun | None:
        """Return the run under a row lock, or None; held until the transaction commits."""
    ...
    async def update_status(
        ...
        *,
        ...
        """Compare-and-set the status; None unless the run is still in ``expected``.
        ...
        frame has already moved on. ``started_at`` is stamped on first entering RUNNING and
        ``ended_at`` on entering a terminal state, both by the backend clock.
        """
    ...
    async def list_active_by_robot(self, robot_id: str) -> list[MissionRunSummary]:
        """Return the runs occupying the robot: PENDING through PAUSED."""
